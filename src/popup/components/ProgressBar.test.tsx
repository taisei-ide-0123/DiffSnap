...
describe('ProgressBar', () => {
  ...
  describe('Error handling', () => {
    ...
    it('shows error list when failed images exist', () => {
      const failedImages = [
        { url: 'https://example.com/image1.jpg', error: 'Network timeout', errorType: 'TIMEOUT' as const, retryCount: 0 },
      ]
      render(
        <ProgressBar
          status="error"
    ...
    it('displays multiple failed images', () => {
      const failedImages = [
        { url: 'https://example.com/image1.jpg', error: 'Network timeout', errorType: 'TIMEOUT' as const, retryCount: 0 },
        { url: 'https://example.com/image2.jpg', error: '404 Not Found', errorType: 'HTTP_ERROR' as const, retryCount: 0 },
        { url: 'https://example.com/image3.jpg', error: 'CORS error', errorType: 'CORS' as const, retryCount: 0 },
      ]
      render(
        <ProgressBar
          status="error"
    ...
    it('truncates long URLs', () => {
      ...
      const failedImages = [{ url: longUrl, error: 'Error', errorType: 'UNKNOWN' as const, retryCount: 0 }]
      render(
        <ProgressBar
          status="error"
          current={99}
    ...
    it('does not truncate short URLs', () => {
      ...
      const failedImages = [{ url: shortUrl, error: 'Error', errorType: 'UNKNOWN' as const, retryCount: 0 }]
      render(
        <ProgressBar
          status="error"
          current={99}
  ...
  describe('Retry functionality', () => {
    it('shows retry button when onRetry is provided', () => {
      const failedImages = [
        { url: 'https://example.com/image1.jpg', error: 'Network error', errorType: 'NETWORK' as const, retryCount: 0 },
      ]
      const onRetry = vi.fn()
      render(
        <ProgressBar
    ...
    it('shows retry button for each failed image', () => {
      const failedImages = [
        { url: 'https://example.com/image1.jpg', error: 'Error 1', errorType: 'UNKNOWN' as const, retryCount: 0 },
        { url: 'https://example.com/image2.jpg', error: 'Error 2', errorType: 'UNKNOWN' as const, retryCount: 0 },
      ]
      const onRetry = vi.fn()
      render(
        <ProgressBar
