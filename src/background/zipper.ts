...
export interface DownloadOptions {
  /**
   * Save as dialog (default: true)
   */
  saveAs?: boolean

  /**
   * URL revoke timeout in milliseconds (default: 60000)
   */
  revokeTimeout?: number
}

/**
 * Download blob via chrome.downloads API
 *
 * Process:
 * 1. Create blob URL with URL.createObjectURL
 * 2. Trigger download with chrome.downloads.download
 * 3. After timeout, revoke blob URL to free memory
 *
...
export const download = async (
  ...
  try {
    ...
    // Schedule URL revocation after timeout
    setTimeout(() => {
